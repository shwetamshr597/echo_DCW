...
class PurchaseBuilder implements \WeltPixel\GA4\Api\ServerSide\Events\PurchaseBuilderInterface
{
    ...
    function getPurchaseEvent($order)
    {
        ...
        $purchaseEvent->setAffiliation($this->ga4Helper->getAffiliationName());
        $purchaseEvent->setCoupon((string)$order->getCouponCode());
        $purchaseEvent->setValue(number_format($this->getOrderTotal($order), 2, '.', ''));
        $purchaseEvent->setShipping(number_format($order->getShippingAmount(), 2, '.', ''));
        $purchaseEvent->setTax(number_format($order->getTaxAmount(), 2, '.', ''));
        ...
        foreach ($order->getAllVisibleItems() as $item) {
            ...
            $productItemOptions['item_name'] = html_entity_decode($item->getName());
            ...
            $productItemOptions['price'] = number_format($item->getPrice(), 2, '.', '');
