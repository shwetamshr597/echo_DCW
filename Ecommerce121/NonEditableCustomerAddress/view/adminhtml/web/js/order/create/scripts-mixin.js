define(
    ...
    function (jQuery) {
        ...
        return function () {
            ...
            window.AdminOrder.prototype.disableFormFields = function () {
                const billingFieldsContainer = jQuery('#order-billing_address_fields');
                const shippingFieldsContainer = jQuery('#order-shipping_address_fields');
                disableContainerFields(jQuery, billingFieldsContainer);
                disableContainerFields(jQuery, shippingFieldsContainer);
            };

            function disableContainerFields($, container) {
                let fieldsDiv = container;
                fieldsDiv.find('input').prop('readonly', true);

                let selectElements = fieldsDiv.find('select');

                selectElements.each(function () {
                    let currentSelect = $(this);
                    let selectedValue = '';
                    if (currentSelect.attr('id').includes('country_id')) {
                        selectedValue = currentSelect.val();
                    } else {
                        selectedValue = currentSelect.attr('defaultvalue');
                    }
